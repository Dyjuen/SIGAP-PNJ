...
<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme" style="z-index: 1000 !important;">
  <div class="app-brand demo">
    <a href="/bendahara/dashboard" class="app-brand-link">
  ...
  <ul class="menu-inner">
    <!-- Dashboard -->
    <li class="menu-item">
      <a href="/bendahara/dashboard" class="menu-link" data-tooltip="Dashboard">
    ...
    <!-- Riwayat -->
    <li class="menu-item">
      <a href="/bendahara/riwayat" class="menu-link" data-tooltip="Riwayat">
        <span class="menu-icon">
...
<script>
  ...
  function loadUserData() {
    console.log('[BENDAHARA SIDEBAR] 🚀 loadUserData() called');
    const storedUserData = localStorage.getItem('userData');
    
    if (storedUserData) {
      try {
        const userData = JSON.parse(storedUserData);
        console.log('[BENDAHARA SIDEBAR] 💾 Using cached data:', userData);
        updateUserProfile(userData);
        fetchUserDataFromAPI();
      } catch (e) {
        console.error('[BENDAHARA SIDEBAR] ❌ Error parsing user data:', e);
        fetchUserDataFromAPI();
      }
    } else {
      console.log('[BENDAHARA SIDEBAR] 📡 No cached data, fetching from API...');
      fetchUserDataFromAPI();
    }
  }
  ...
  function fetchUserDataFromAPI() {
    ...
    console.log('[BENDAHARA SIDEBAR] 🔄 Fetching user data from API...');
    console.log('[BENDAHARA SIDEBAR] 🔑 Token exists:', !!token);
    
    if (!token) {
      console.error('[BENDAHARA SIDEBAR] ❌ No authentication token found');
      updateUserProfile({ name: 'User', email: 'user@email.com', role: 'Bendahara' });
      return;
    }
    ...
    fetch('/api/auth/profile', {
      ...
      .then(response => {
        console.log('[BENDAHARA SIDEBAR] 📥 API Response status:', response.status);
        if (!response.ok) throw new Error('API responded with status: ' + response.status);
        return response.json();
      })
      .then(result => {
        console.log('[BENDAHARA SIDEBAR] ✅ API Result:', result);
        
        if (result.success && result.data) {
          const userData = {
            name: result.data.nama_lengkap || 'User',
            email: result.data.email || 'user@email.com',
            role: (result.data.roles && result.data.roles[0]) || result.data.role_name || 'Bendahara',
            username: result.data.username || ''
          };
          
          console.log('[BENDAHARA SIDEBAR] 📦 User data prepared:', userData);
          localStorage.setItem('userData', JSON.stringify(userData));
          updateUserProfile(userData);
          console.log('[BENDAHARA SIDEBAR] ✨ Profile updated successfully!');
        } else {
          throw new Error('Invalid API response format');
        }
      })
      .catch(error => {
        console.error('[BENDAHARA SIDEBAR] ❌ Error fetching user data:', error);
        updateUserProfile({ name: 'User', email: 'user@email.com', role: 'Bendahara' });
      });
  }
  ...
  function updateUserProfile(userData) {
    console.log('[BENDAHARA SIDEBAR] 🎨 Updating profile UI with:', userData);
    ...
    console.log('[BENDAHARA SIDEBAR] 🎉 Profile update complete!');
  }
  ...
  console.log('[BENDAHARA SIDEBAR] 🛠️ Debug tools ready! Available via window.bendaharaSidebarDebug');
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() {
      console.log('[BENDAHARA SIDEBAR] ⏰ Delayed load triggered');
      loadUserData();
    }, 200);
  ...
  window.addEventListener('load', function() {
    console.log('[BENDAHARA SIDEBAR] 🪟 Window load fired - checking data...');
    const userNameEl = document.getElementById('user-name');
    if (userNameEl && !userNameEl.textContent.trim()) {
      console.log('[BENDAHARA SIDEBAR] ⚠️ User name still empty, forcing reload...');
      loadUserData();
    } else if (userNameEl) {
      console.log('[BENDAHARA SIDEBAR] ✅ User name already populated:', userNameEl.textContent);
    }
  });
