...
<script>
  // ============= REKTORAT SIDEBAR API INTEGRATION =============
  // Global functions for user data management
  function loadUserData() {
    console.log('[REKTORAT SIDEBAR] 🚀 Starting loadUserData...');
    const storedUserData = localStorage.getItem('userData');
    
    if (storedUserData) {
      console.log('[REKTORAT SIDEBAR] ✅ Found cached user data');
      try {
        const userData = JSON.parse(storedUserData);
        updateUserProfile(userData);
      } catch (e) {
        console.error('[REKTORAT SIDEBAR] ❌ Error parsing cached data:', e);
      }
    }
    
    // Always fetch fresh data from API
    console.log('[REKTORAT SIDEBAR] 🌐 Fetching fresh data from API...');
  ...
  function fetchUserDataFromAPI() {
    ...
    if (!token) {
      console.error('[REKTORAT SIDEBAR] ❌ No token found');
      return;
    }
    ...
    fetch('/api/auth/profile', {
      ...
      .then(response => {
        console.log('[REKTORAT SIDEBAR] 📥 Response status:', response.status);
        return response.json();
      })
      .then(result => {
        console.log('[REKTORAT SIDEBAR] 📦 API Response:', result);
        ...
        if (result.success && result.data) {
          ...
          console.log('[REKTORAT SIDEBAR] ✅ Mapped userData:', userData);
          localStorage.setItem('userData', JSON.stringify(userData));
          updateUserProfile(userData);
        } else {
          console.error('[REKTORAT SIDEBAR] ❌ Invalid API response structure');
        }
      })
      .catch(error => {
        console.error('[REKTORAT SIDEBAR] ❌ API Error:', error);
      });
  }

  function updateUserProfile(userData) {
    console.log('[REKTORAT SIDEBAR] 🎨 Updating UI with:', userData);
  ...
  window.addEventListener('load', function() {
    console.log('[REKTORAT SIDEBAR] 🔄 Window loaded - checking if data populated');
    const userNameEl = document.getElementById('user-name');
    if (userNameEl && !userNameEl.textContent.trim()) {
      console.log('[REKTORAT SIDEBAR] ⚠️ Name empty on window.load, retrying...');
      loadUserData();
    }
  });
  ...
  window.rektoratSidebarDebug = {
    ...
    checkElements: function() {
      console.log('[REKTORAT SIDEBAR DEBUG] 🔍 Element check:');
  ...
  console.log('[REKTORAT SIDEBAR] ✅ Debug tools ready! Use window.rektoratSidebarDebug');
</script>
`;
