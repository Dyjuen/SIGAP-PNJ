...
<script>
  ...
  function loadUserData() {
    console.log('[PPK SIDEBAR] 🚀 loadUserData() called');
    const storedUserData = localStorage.getItem('userData');
    
    if (storedUserData) {
      try {
        const userData = JSON.parse(storedUserData);
        console.log('[PPK SIDEBAR] 💾 Using cached data:', userData);
        updateUserProfile(userData);
        fetchUserDataFromAPI();
      } catch (e) {
        console.error('[PPK SIDEBAR] ❌ Error parsing user data:', e);
        fetchUserDataFromAPI();
      }
    } else {
      console.log('[PPK SIDEBAR] 📡 No cached data, fetching from API...');
      fetchUserDataFromAPI();
    }
  }
  ...
  function fetchUserDataFromAPI() {
    ...
    console.log('[PPK SIDEBAR] 🔄 Fetching user data from API...');
    console.log('[PPK SIDEBAR] 🔑 Token exists:', !!token);
    
    if (!token) {
      console.error('[PPK SIDEBAR] ❌ No authentication token found');
      updateUserProfile({ name: 'User', email: 'user@email.com', role: 'PPK' });
      return;
    }
    ...
    fetch('/api/auth/profile', {
      ...
      .then(response => {
        console.log('[PPK SIDEBAR] 📥 API Response status:', response.status);
        if (!response.ok) throw new Error('API responded with status: ' + response.status);
        return response.json();
      })
      .then(result => {
        console.log('[PPK SIDEBAR] ✅ API Result:', result);
        
        if (result.success && result.data) {
          const userData = {
            name: result.data.nama_lengkap || 'User',
            email: result.data.email || 'user@email.com',
            role: (result.data.roles && result.data.roles[0]) || result.data.role_name || 'PPK',
            username: result.data.username || ''
          };
          
          console.log('[PPK SIDEBAR] 📦 User data prepared:', userData);
          localStorage.setItem('userData', JSON.stringify(userData));
          updateUserProfile(userData);
          console.log('[PPK SIDEBAR] ✨ Profile updated successfully!');
        } else {
          throw new Error('Invalid API response format');
        }
      })
      .catch(error => {
        console.error('[PPK SIDEBAR] ❌ Error fetching user data:', error);
        updateUserProfile({ name: 'User', email: 'user@email.com', role: 'PPK' });
      });
  }
  ...
  function updateUserProfile(userData) {
    console.log('[PPK SIDEBAR] 🎨 Updating profile UI with:', userData);
    ...
    console.log('[PPK SIDEBAR] 🎉 Profile update complete!');
  }
  ...
  console.log('[PPK SIDEBAR] 🛠️ Debug tools ready! Available via window.ppkSidebarDebug');
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() {
      console.log('[PPK SIDEBAR] ⏰ Delayed load triggered');
      loadUserData();
    }, 200);
  ...
  window.addEventListener('load', function() {
    console.log('[PPK SIDEBAR] 🪟 Window load fired - checking data...');
    const userNameEl = document.getElementById('user-name');
    if (userNameEl && !userNameEl.textContent.trim()) {
      console.log('[PPK SIDEBAR] ⚠️ User name still empty, forcing reload...');
      loadUserData();
    } else if (userNameEl) {
      console.log('[PPK SIDEBAR] ✅ User name already populated:', userNameEl.textContent);
    }
  });
