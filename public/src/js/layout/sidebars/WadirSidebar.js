...
<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme" style="z-index: 1000 !important;">
  <div class="app-brand demo">
    <a href="/wadir/dashboard" class="app-brand-link">
  ...
  <ul class="menu-inner">
    <!-- Dashboard -->
    <li class="menu-item">
      <a href="/wadir/dashboard" class="menu-link" data-tooltip="Dashboard">
...
<script>
  ...
  function loadUserData() {
    console.log('[WADIR SIDEBAR] 🚀 loadUserData() called');
    const storedUserData = localStorage.getItem('userData');
    
    if (storedUserData) {
      try {
        const userData = JSON.parse(storedUserData);
        console.log('[WADIR SIDEBAR] 💾 Using cached data:', userData);
        updateUserProfile(userData);
        fetchUserDataFromAPI();
      } catch (e) {
        console.error('[WADIR SIDEBAR] ❌ Error parsing user data:', e);
        fetchUserDataFromAPI();
      }
    } else {
      console.log('[WADIR SIDEBAR] 📡 No cached data, fetching from API...');
      fetchUserDataFromAPI();
    }
  }
  ...
  function fetchUserDataFromAPI() {
    ...
    console.log('[WADIR SIDEBAR] 🔄 Fetching user data from API...');
    console.log('[WADIR SIDEBAR] 🔑 Token exists:', !!token);
    
    if (!token) {
      console.error('[WADIR SIDEBAR] ❌ No authentication token found');
      updateUserProfile({ name: 'User', email: 'user@email.com', role: 'Wadir' });
      return;
    }
    
    console.log('[WADIR SIDEBAR] 📡 Calling /api/auth/profile...');
    fetch('/api/auth/profile', {
      ...
      .then(response => {
        console.log('[WADIR SIDEBAR] 📥 API Response status:', response.status);
        if (!response.ok) throw new Error('API responded with status: ' + response.status);
        return response.json();
      })
      .then(result => {
        console.log('[WADIR SIDEBAR] ✅ API Result:', result);
        
        if (result.success && result.data) {
          const userData = {
            name: result.data.nama_lengkap || 'User',
            email: result.data.email || 'user@email.com',
            role: (result.data.roles && result.data.roles[0]) || result.data.role_name || 'Wadir',
            username: result.data.username || ''
          };
          
          console.log('[WADIR SIDEBAR] 📦 User data prepared:', userData);
          localStorage.setItem('userData', JSON.stringify(userData));
          updateUserProfile(userData);
          console.log('[WADIR SIDEBAR] ✨ Profile updated successfully!');
        } else {
          throw new Error('Invalid API response format');
        }
      })
      .catch(error => {
        console.error('[WADIR SIDEBAR] ❌ Error fetching user data:', error);
        updateUserProfile({ name: 'User', email: 'user@email.com', role: 'Wadir' });
      });
  }
  ...
  function updateUserProfile(userData) {
    console.log('[WADIR SIDEBAR] 🎨 Updating profile UI with:', userData);
    ...
    if (userData.name && userNameEl) {
      ...
      console.log('[WADIR SIDEBAR] ✅ Updated name:', userData.name);
      ...
      if (userAvatarEl) {
        ...
        console.log('[WADIR SIDEBAR] ✅ Updated avatar:', initials);
      }
    ...
    if (userData.email && userEmailEl) {
      ...
      if (emailSpan) {
        ...
        console.log('[WADIR SIDEBAR] ✅ Updated email:', userData.email);
      }
    }
    
    if (userData.role && userRoleEl) {
      userRoleEl.textContent = userData.role;
      console.log('[WADIR SIDEBAR] ✅ Updated role:', userData.role);
    }
    
    console.log('[WADIR SIDEBAR] 🎉 Profile update complete!');
  }
  
  // Expose functions to window IMMEDIATELY
  window.wadirSidebarDebug = {
    loadUserData: loadUserData,
    fetchUserDataFromAPI: fetchUserDataFromAPI,
    updateUserProfile: updateUserProfile
  };
  
  console.log('[WADIR SIDEBAR] 🛠️ Debug tools ready! Available via window.wadirSidebarDebug');
  
  // ============== DOM EVENT LISTENERS ==============
  
  // Sidebar toggle functionality
  document.addEventListener('DOMContentLoaded', function() {
    console.log('[WADIR SIDEBAR] 🎬 DOMContentLoaded fired');
    ...
    setTimeout(function() {
      console.log('[WADIR SIDEBAR] ⏰ Delayed load triggered');
      loadUserData();
    }, 200);
  ...
  window.addEventListener('load', function() {
    console.log('[WADIR SIDEBAR] 🪟 Window load fired - checking data...');
    const userNameEl = document.getElementById('user-name');
    if (userNameEl && !userNameEl.textContent.trim()) {
      console.log('[WADIR SIDEBAR] ⚠️ User name still empty, forcing reload...');
      loadUserData();
    } else if (userNameEl) {
      console.log('[WADIR SIDEBAR] ✅ User name already populated:', userNameEl.textContent);
    }
  });
