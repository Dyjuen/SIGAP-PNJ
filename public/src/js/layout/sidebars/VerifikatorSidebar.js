...
<script>
  // ============= VERIFIKATOR SIDEBAR API INTEGRATION =============
  // Global functions for user data management
  function loadUserData() {
    console.log('[VERIFIKATOR SIDEBAR] 🚀 Starting loadUserData...');
    const storedUserData = localStorage.getItem('userData');
    
    if (storedUserData) {
      console.log('[VERIFIKATOR SIDEBAR] ✅ Found cached user data');
      try {
        const userData = JSON.parse(storedUserData);
        updateUserProfile(userData);
      } catch (e) {
        console.error('[VERIFIKATOR SIDEBAR] ❌ Error parsing cached data:', e);
      }
    }
    
    // Always fetch fresh data from API
    console.log('[VERIFIKATOR SIDEBAR] 🌐 Fetching fresh data from API...');
  ...
  function fetchUserDataFromAPI() {
    ...
    if (!token) {
      console.error('[VERIFIKATOR SIDEBAR] ❌ No token found');
      return;
    }
    ...
    fetch('/api/auth/profile', {
      ...
      .then(response => {
        console.log('[VERIFIKATOR SIDEBAR] 📥 Response status:', response.status);
        return response.json();
      })
      .then(result => {
        console.log('[VERIFIKATOR SIDEBAR] 📦 API Response:', result);
        ...
        if (result.success && result.data) {
          ...
          console.log('[VERIFIKATOR SIDEBAR] ✅ Mapped userData:', userData);
          localStorage.setItem('userData', JSON.stringify(userData));
          updateUserProfile(userData);
        } else {
          console.error('[VERIFIKATOR SIDEBAR] ❌ Invalid API response structure');
        }
      })
      .catch(error => {
        console.error('[VERIFIKATOR SIDEBAR] ❌ API Error:', error);
      });
  }

  function updateUserProfile(userData) {
    console.log('[VERIFIKATOR SIDEBAR] 🎨 Updating UI with:', userData);
  ...
  window.addEventListener('load', function() {
    console.log('[VERIFIKATOR SIDEBAR] 🔄 Window loaded - checking if data populated');
    const userNameEl = document.getElementById('user-name');
    if (userNameEl && !userNameEl.textContent.trim()) {
      console.log('[VERIFIKATOR SIDEBAR] ⚠️ Name empty on window.load, retrying...');
      loadUserData();
    }
  });
  ...
  window.verifikatorSidebarDebug = {
    ...
    checkElements: function() {
      console.log('[VERIFIKATOR SIDEBAR DEBUG] 🔍 Element check:');
  ...
  console.log('[VERIFIKATOR SIDEBAR] ✅ Debug tools ready! Use window.verifikatorSidebarDebug');
</script>
`;
