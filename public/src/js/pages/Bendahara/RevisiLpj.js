...
<!-- Comment Detail Modal -->
  ...
  // Fungsi untuk membuka modal detail catatan - Adapted for RevisiLpj
  window.openCommentDetailModal = function() {
    ...
    const commentsList = [];
    for (const [id, comment] of Object.entries(lampiranComments)) {
        // Find file info from state.lampiran
        const file = state.lampiran.find(f => f.lampiran_id == id);
        const filename = file ? file.nama_file_asli : "Unknown File";
        commentsList.push({ id, filename, comment });
    }
    ...
    if (commentsList.length > 0) {
        ...
        commentsList.forEach(item => {
            html += `
                <div class="comment-item" onclick="navigateToLampiran('${item.id}')">
    ...
    if (!hasComments) {
        html = `
            <div class="comment-empty-state">
                ...
                <div class="empty-subtitle">Belum ada catatan yang ditambahkan untuk LPJ ini.</div>
            </div>
        `;
    }
  ...
  window.navigateToLampiran = function(lampiranId) {
      const modalEl = document.getElementById('commentDetailModal');
      const modal = bootstrap.Modal.getInstance(modalEl);
      if (modal) modal.hide();

      setTimeout(() => {
          const targetElement = document.querySelector(`.lampiran-item[data-lampiran-id="${lampiranId}"]`);
          if (targetElement) {
              const yOffset = -150;
